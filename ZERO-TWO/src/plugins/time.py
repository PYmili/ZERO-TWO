...
sj = on_keyword({"现在时间","现在几点","多少时间","时间多少","时间","几点"}, to_me())
...
async def bxxx_r(bot: Bot, event: Event, state: T_State):
    ...
    await sj.send(MessageSegment.at(id) + "现在时间是：" + time2)

'''----------------今天星期几-------------------'''
xqj = on_keyword({"星期几"}, to_me())
@xqj.handle()
async def xqj_r(bot: Bot, event: Event, state: T_State):
    ...
    js = "\n(数字1-7代表周一到周日)"
    await xqj.send(MessageSegment.at(id) + "今天是星期" + dayOfWeek + js)    

'''----------------------年月日---------------------'''
nian = on_keyword({"几年","几月","几日","几时","几分","几秒"}, to_me())
@nian.handle()
async def nian_r(bot: Bot, event: Event, state: T_State):
    ...
    js = "(年-月-日_时-分-秒)"
    js2 = "\n(数字1-7代表周一到周日)"
    await nian.send(MessageSegment.at(id) + "什么你不会看时间吗？真麻烦！现在是：" + time2 + js + "\n星期：" + dayOfWeek + js2)
